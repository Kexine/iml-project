...
9)	Model selection was performed in [model_selection.ipynb](model_selection.ipynb) with early stopping using a disjoint validation set. We found using a disjoint validation set to be crucial for model selection. There, we tried out different model architectures. Only with the best found architecture we trained on the (almost) entire training dataset in [main.ipynb](main.ipynb).
